"use strict";
// Parse the Data
d3.csv("/static/Simplified_C-CAP_Scheme.csv").then((data) => {
  // List of subgroups = header of the csv files = soil condition here
  const subgroups = data.columns.slice(5);
  // console.log(subgroups);

  // *Grabbing CCAP Groups from CSV Column to be used on X Axis
  const groups = [...new Set(data.map((d) => d.C_CAP))];
  ...
  const margin = { top: 40, right: 80, bottom: 315, left: 100 },
  ...
  // Add Y axis
  const yAxis = d3.scaleLinear().domain([0, 300]).range([height, 0]);
  ...
  // *Starting modifications for tooltip here
  const tooltip = d3
    .select("#my_dataviz_vert")
    .append("div")
    .style("opacity", 0)
  ...
  svg
    ...
    .enter()
    .append("g")
    .attr("transform", (d) => "translate(" + xAxis(d.C_CAP) + ",0)")
    .selectAll("rect")
    .data((d) => {
